...
We can't easily trigger `UINT32_MAX` dead bytes in a test, but we can fix the code and add a targeted unit test that verifies compaction correctness when dead bytes equal pool_used (the `live_size == 0` early-exit path):
...
**Step 2: Implement fix — compute true live size by scanning elements**

Replace the `live_size` estimation in `td_str_vec_compact` with a scan:
...
td_t* td_str_vec_compact(td_t* vec) {
    ...
    /* Compute true live size by scanning elements */
...
This eliminates the dead-counter-based `live_size` estimation entirely and always computes the true value. The dead counter is still useful as a **trigger** (the early-exit `str_pool_dead(vec) == 0` check at the top), but the allocation size is now always correct.
...
git commit -m "fix: compact computes true live size by scanning — prevents overflow on dead counter saturation"
```
